...
func _build_catalogue() -> void:
	# Weapons
	var weap2 = EquipmentItem.new("Stick Staff", EquipmentItem.Slot.WEAPON)
	weap2.attack_bonus_pct = 0.5
	weap2.calculate_cost()

	var weap3 = EquipmentItem.new("Basic Staff", EquipmentItem.Slot.WEAPON)
	weap3.attack_bonus_pct = 0.8
	weap3.crit_rate_bonus_pct = 0.2
	weap3.calculate_cost()

	var weap4 = EquipmentItem.new("Magical Staff", EquipmentItem.Slot.WEAPON)
	weap4.attack_bonus_pct = 1.2
	weap4.crit_dmg_bonus_pct = 0.3
	weap4.calculate_cost()

	var weap5 = EquipmentItem.new("Great Staff", EquipmentItem.Slot.WEAPON)
	...
	# Robes
	var robe2 = EquipmentItem.new("Rags", EquipmentItem.Slot.ROBE)
	robe2.ingredient_gain_bonus_pct = 0.5
	robe2.calculate_cost()

	var robe3 = EquipmentItem.new("Basic Robe", EquipmentItem.Slot.ROBE)
	robe3.ingredient_gain_bonus_pct = 1.0
	robe3.health_bonus_pct = 0.2
	robe3.calculate_cost()

	var robe4 = EquipmentItem.new("Master Robe", EquipmentItem.Slot.ROBE)
	...
	# Hats
	var hat2 = EquipmentItem.new("Bucket Hat", EquipmentItem.Slot.HAT)
	hat2.crit_rate_bonus_pct = 0.5
	hat2.calculate_cost()

	var hat3 = EquipmentItem.new("Slightly Magical Hat", EquipmentItem.Slot.HAT)
	hat3.crit_rate_bonus_pct = 1.0
	hat3.crit_dmg_bonus_pct = 0.3
	hat3.calculate_cost()

	var hat4 = EquipmentItem.new("Wizard Hat", EquipmentItem.Slot.HAT)
	hat4.crit_rate_bonus_pct = 1.5
	hat4.crit_dmg_bonus_pct = 0.5
	hat4.attack_bonus_pct = 0.2
	hat4.calculate_cost()

	var hat5 = EquipmentItem.new("Rabadon's Deathcap", EquipmentItem.Slot.HAT)
