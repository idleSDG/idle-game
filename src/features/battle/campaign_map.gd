...
func _init(
		p_levels: Array[battle_level],
		p_name: String,
		p_generate_paths: bool,
	...
	levels = []
	paths = []
	p_levels.sort_custom(
		func(level1: battle_level, level2: battle_level):
			if level1.depth == level2.depth:
				return level1.position < level2.position
			else:
				return level1.depth < level2.depth
	)
	levels = Array(p_levels)
	name = p_name
	if p_generate_paths:
		generate_paths(existing_paths)
	background_image = p_background_image
	background_color = p_background_color
	type = p_type


func generate_paths(existing_paths: Array[battle_path]) -> void:
	...
	for current_level in levels:
		if (current_level.depth == max_depth):
			break
		var existing_level_paths = existing_paths.filter(func(x): return x.start == current_level)
		if !existing_level_paths.is_empty():
			for existing_path in existing_level_paths:
...
static func generate_map(
	...
	for i in range(depth):
		...
		if !existing_depth_levels.is_empty():
			...
			for j in range(last_position_count):
	...
	return campaign_map.new(campaign_levels, p_name, true, existing_paths, p_background_image, p_background_color, p_type)
