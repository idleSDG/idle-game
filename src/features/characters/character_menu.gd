...
# Stats labels
const STATS_BASE := "CanvasLayer/Background/VBox/HBoxBottom/Stats/StatsSection/Margin/Margin/Margin/StatsGrid"
...
@onready var color_picker     : ColorPicker   = $CanvasLayer/Background/SkinColorPicker/SkinColorPanel/VBoxContainer/ColorPicker
...
func _ready() -> void:
	...
	skin_btn.pressed.connect(func(): print("skin pressed via lambda"))
...
func _refresh_stats() -> void:
	...
	_set_stat(hp_final_label,
		_fmt_int(final.maxHealth, final.maxHealth - base.maxHealth),
		final.maxHealth != base.maxHealth)

	_set_stat(atk_final_label,
		_fmt_int(final.attack, final.attack - base.attack),
		final.attack != base.attack)

	_set_stat(def_final_label,
		_fmt_int(final.defense, final.defense - base.defense),
		final.defense != base.defense)

	_set_stat(crit_final_label,
		_fmt_pct(final.critRate, final.critRate - base.critRate),
		final.critRate != base.critRate)

	_set_stat(critdmg_final_label,
		_fmt_pct(1.0 + final.critDMG, final.critDMG - base.critDMG),
		final.critDMG != base.critDMG)

	_set_stat(ing_final_label,
		_fmt_pct(final_ing, bonuses["ingredient_gain_pct"]),
		final_ing != 1.0)
...
func _on_skin_btn_pressed() -> void:
	skin_color_picker.visible = !skin_color_picker.visible
