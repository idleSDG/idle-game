...
@onready var weapon_container: VBoxContainer = $CanvasLayer/TabContainer/Weapons/ScrollContainer/ItemList
...
func _populate() -> void:
	for item in ShopCatalogue.items:
		...
		match item.slot:
			EquipmentItem.Slot.WEAPON: weapon_container.add_child(card)
			EquipmentItem.Slot.ROBE: robe_container.add_child(card)
			EquipmentItem.Slot.HAT: hat_container.add_child(card)
		card.setup(item)
